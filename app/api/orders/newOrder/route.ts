...
const ORDER_ID_PADDING = 4;

export async function POST(request: Request) {
  try {
    // Parse request body
    const { sessionId, tableNumber, foodItems, guestName } =
      await request.json();
    ...
    const invalidFoodItems = foodItems.some(
      (item) =>
        !item.foodId ||
        !item.name ||
        !item.quantity ||
    ...
    if (invalidFoodItems) {
      ...
      return NextResponse.json(
        {
          message:
            "Each food item must have foodId, name, quantity, price and category",
        },
        { status: 400 },
      );
    }
    ...
    // Verify the session is one of the active seats at this table
    const table = await Tables.findOne({
      tableNumber,
      occupiedBy: { $in: [sessionId] },
      status: "occupied",
    });

    if (!table) {
      console.error(
        "Unauthorized request: Invalid table number or session ID:",
        { tableNumber, sessionId },
      );
      return NextResponse.json(
        { message: "Unauthorized: Invalid table number or session ID" },
        { status: 403 },
      );
    }
    ...
    const order = {
      ...
      tableNumber,
      guestName:
        typeof guestName === "string" ? guestName.trim().slice(0, 50) : "",
      foodItems,
      status: "ordered",
      payment: "unpaid",
      timestamps: {
    ...
    try {
      ...
      return NextResponse.json(
        { message: "Order validation failed", error: validationError },
        { status: 400 },
      );
    }
    ...
    if (result) {
      log({
        ...
        details: {
          ...
          items: foodItems.map((i: { name: string }) => i.name),
        },
      });
