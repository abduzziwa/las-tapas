...
const Login: React.FC = () => {
  ...
  const handleLogin = async () => {
    if (!employeeId) {
      setError(
        "Employee ID is required to clock in.",
        setErrorMessage,
        setCountdown
      );
      return;
    ...
    const response = await handleAction(
      ...
      (message) => setError(message, setErrorMessage, setCountdown)
    );
  ...
  const handleLogout = async () => {
    if (!employeeId) {
      setError(
        "Employee ID is required to clock out.",
        setErrorMessage,
        setCountdown
      );
      return;
    ...
    await handleAction("clockOut", { shiftId }, (message) =>
      setError(message, setErrorMessage, setCountdown)
    );
  ...
  const handleBreak = async () => {
    if (!employeeId) {
      setError(
        "Employee ID is required to start or end a break.",
        setErrorMessage,
        setCountdown
      );
      return;
    ...
    if (!shiftId) {
      setError(
        "You must be clocked in to start or end a break.",
        setErrorMessage,
        setCountdown
      );
      return;
    ...
    if (isBreakActive) {
      const response = await handleAction("endBreak", { breakId }, (message) =>
        setError(message, setErrorMessage, setCountdown)
      );
      if (response && response.success) {
      ...
      const response = await handleAction(
        "startBreak",
        { employeeId: "e" + employeeId, shiftId },
        (message) => setError(message, setErrorMessage, setCountdown)
      );
      if (response && response.success) {
