...
export default function Foods() {
  ...
  const [drinksCartItems, setCartItems] = useState<OrderItem[]>([]); // State to store the cart items
    ...
    const fetchMeals = async () => {
      try {
        const response = await fetch(
          "http://192.168.178.12:3000/api/menu/drinks"
        );
        if (!response.ok) {
  ...
  return (
    <main>
      ...
      <div className="order-summary p-[24px] mt-0 pt-3 pb-5">
        {drinksCartItems.length > 0 ? (
          drinksCartItems.map((item, index) => (
